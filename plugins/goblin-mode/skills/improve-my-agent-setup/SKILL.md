...
description: Audit and improve someone's whole agentic-development setup, then offer to fix what's weak. This is the Goblin Mode setup audit. Invoke whenever the user asks to "audit my setup", "review my agentic setup", "how's my Claude Code setup", "am I set up well", "check my skills / memory / context", "what should I improve", "grade my workflow", "is my setup any good", "goblin mode", or wonders whether they have the right skills, tools, memory, statusline, models, or habits, or whether they have secrets or PII sitting in plaintext. Accepts a mode argument (shallow, deep, ultra, yolo) setting depth and tone from a quick glance to a drill-sergeant demolition. Covers the full picture: skills inventory and overlap, context ownership, memory budget and organization, tools and redundancy, speech-to-text input, local/open-source model independence, other clients (Claude Desktop, VS Code, Cursor, Zed), harness config (statusline, hooks, permissions), permission mode and isolation, safety enforcement, supply-chain guards, git logging, model and effort economy, usage review, delegation ethics, determinism, shipping focus, automation, and secrets/PII hygiene. Reviews project context files too (this absorbs the old /audit-context) and can schedule itself to re-run. Route here for a setup-wide health check, not for auditing a single semantic model, report, or tenant.
---
