...
| Treat calculated columns as storage-mode-specific | DirectQuery allows row-level expressions that fold; Direct Lake on SQL does not support calculated columns that reference Direct Lake tables, while Direct Lake on OneLake supports unmaterialized calculated columns in preview. Prefer upstream columns when portability matters | `te add Sales/Bucket -t CalculatedColumn -i "<DAX>" --save` |
